...
export class PlatosControlador {
  ...
  obtenerPlatos = async (request: FastifyRequest<{ Querystring: { limite?: number } }>, reply: FastifyReply) => {
    try {
  ...
  obtenerPlatoPorId = async (request: FastifyRequest<{ Params: { idPlato: string } }>, reply: FastifyReply) => {
    try {
  ...
  actualizarPlato = async (
    ...
    try {
      ...
      const platoActualizado = await this.platosCasosUso.actualizarPlato(idPlato, nuevoPlato);
